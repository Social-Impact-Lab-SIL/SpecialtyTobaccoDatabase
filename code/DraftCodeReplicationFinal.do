...
**Summary**
use /cluster/VAST/mp94p-lab/Landini/GoogleLocations/BaseFiles/GoogleTractVapeSmokeCigarFinal, replace


estpost sum tractvape tracttobacco tractcigar age1524pct minoritypct femalepct povertypct HSorLess [aweight = Population]

sum rural Population
...
***Fig 3 For only MML/RML States***
destring state, gen(statefip)
merge m:1 statefip using "/cluster/VAST/mp94p-lab/reusable_files/Marijuana/RML MML dates 2025.dta", keepusing(mml* rml*)
drop _merge 
gen rmlmml = 1 if mml_year != . | rml_year != . 
estimates drop _all 
areg tractvapeper age1524pct minoritypct femalepct povertypct rural HSorLess [aweight = Population] if rmlmml == 1, absorb(state) vce(robust)
est sto m1
areg tracttobaccoper age1524pct minoritypct femalepct povertypct rural HSorLess [aweight = Population] if rmlmml == 1, absorb(state) vce(robust)
est sto m2
areg tractcigarper age1524pct minoritypct femalepct povertypct rural HSorLess [aweight = Population] if rmlmml == 1, absorb(state) vce(robust)
est sto m3
*areg tracthookahper age1524pct minoritypct femalepct povertypct rural HSorLess [aweight = Population], absorb(state) vce(robust)
*est sto m4 
...
***Fig 3 version including disaggregated races***
merge 1:1 fips using"/cluster/VAST/mp94p-lab/Landini/GoogleLocations/ACSTract19.dta", keepusing(Latinx NonHispanicWA NonHispanicBA NonHispanicAA)
drop _merge 
rename Latinx Hispanic
gen NonHispanic = Population-Hispanic
gen NonHispanicOther = NonHispanic-NonHispanicWA-NonHispanicBA-NonHispanicAA

foreach v of varlist Hispanic NonHispanic NonHispanicWA NonHispanicBA NonHispanicAA NonHispanicOther{
gen `v'pct = (`v'/Population)*100
}

sum NonHispanic*

estimates drop _all 
areg tractvapeper age1524pct Hispanicpct NonHispanicBApct NonHispanicAApct NonHispanicOtherpct femalepct povertypct rural HSorLess [aweight = Population], absorb(state) vce(robust)
est sto m1
areg tracttobaccoper age1524pct Hispanicpct NonHispanicBApct NonHispanicAApct NonHispanicOtherpct femalepct povertypct rural HSorLess [aweight = Population], absorb(state) vce(robust)
est sto m2
areg tractcigarper age1524pct Hispanicpct NonHispanicBApct NonHispanicAApct NonHispanicOtherpct femalepct povertypct rural HSorLess [aweight = Population], absorb(state) vce(robust)
est sto m3

label var Hispanicpct "Hispanic %"
label var NonHispanicWApct "White %"
label var NonHispanicBApct "Black %"
label var NonHispanicAApct "Asian %"
label var NonHispanicOtherpct "Other %"
label var rural "Rural"

coefplot (m1, label(Vape Shops) msymbol(circle)) (m2, label(Smoke Shops) msymbol(diamond)) (m3, label(Cigar Shops) msymbol(square)) /*(m4, label(Hookah) msymbol(triangle))*/, levels(95) drop(_cons femalepct) vertical yline(0, lwidth(thin)) title("", margin(0 0 2 0)) xlab(,labsize(small)) ylab(,labsize(small)) xline(1.5 2.5 3.5 4.5 5.5, lpattern(-) lcolor(gs15)) msize(small) legend(pos(6) row(1))

***Checking Margins for Fig 3***
* Model 1: Vape Shop Density
areg tractvapeper c.povertypct age1524pct minoritypct femalepct rural HSorLess ///
    [aweight = Population], absorb(state) vce(robust)
margins, at(povertypct=(10 20 30 40 50))
marginsplot, title("") name(vape_dens, replace) ytitle("Predicted Smoke Shops") xtitle("Poverty %")

* Model 2: Tobacco Shop Density
areg tracttobaccoper c.povertypct age1524pct minoritypct femalepct rural HSorLess ///
    [aweight = Population], absorb(state) vce(robust)
margins, at(povertypct=(10 20 30 40 50))
marginsplot, title("") name(tob_dens, replace) ytitle("Predicted Smoke Shops") xtitle("Poverty %")

* Model 3: Cigar Shop Density
areg tractcigarper c.povertypct age1524pct minoritypct femalepct rural HSorLess ///
    [aweight = Population], absorb(state) vce(robust)
margins, at(povertypct=(10 20 30 40 50))
marginsplot, title("") name(cig_dens, replace) ytitle("Predicted Cigar Shops") xtitle("Poverty %")
...
**Summary**
estpost sum tractvapevisitors tracttobaccovisitors tractcigarvisitors [aweight = Population]
...
***Tract Level Equivalent of Table 2***
foreach v in tractvapevisitors tracttobaccovisitors tractcigarvisitors tractvapevisitorsper tracttobaccovisitorsper tractcigarvisitorsper {
	gen ln`v' = ln(`v')
	replace ln`v' = 0 if ln`v'== . 
}
estimates drop _all 
areg lntractvapevisitors age1524pct minoritypct femalepct povertypct rural HSorLess [aweight = Population], absorb(state) vce(robust)
est sto m1
areg lntracttobaccovisitors age1524pct minoritypct femalepct povertypct rural HSorLess [aweight = Population], absorb(state) vce(robust)
est sto m2
areg lntractcigarvisitors age1524pct minoritypct femalepct povertypct rural HSorLess [aweight = Population], absorb(state) vce(robust)
est sto m3

coefplot (m1, label(Vape) msymbol(circle)) (m2, label(Tobacco) msymbol(diamond)) (m3, label(Cigar) msymbol(square)), levels(95) drop(_cons femalepct) vertical yline(0, lwidth(thin)) title("", margin(0 0 2 0)) xlab(,labsize(small)) ylab(,labsize(small)) xline(1.5 2.5 3.5 4.5 5.5, lpattern(-) lcolor(gs15)) msize(small) 

esttab m1 m2 m3 using TractTable3.tex, tex replace b(%5.3f) se(%5.3f) star(* .1 ** .05 *** .01) label



estimates drop _all 
areg lntractvapevisitorsper age1524pct minoritypct femalepct povertypct rural HSorLess [aweight = Population], absorb(state) vce(robust)
est sto m1
areg lntracttobaccovisitorsper age1524pct minoritypct femalepct povertypct rural HSorLess [aweight = Population], absorb(state) vce(robust)
est sto m2
areg lntractcigarvisitorsper age1524pct minoritypct femalepct povertypct rural HSorLess [aweight = Population], absorb(state) vce(robust)
est sto m3

coefplot (m1, label(Vape Shops) msymbol(circle)) (m2, label(Smoke Shops) msymbol(diamond)) (m3, label(Cigar Shops) msymbol(square)), levels(95) drop(_cons femalepct) vertical yline(0, lwidth(thin)) title("", margin(0 0 2 0)) xlab(,labsize(small)) ylab(,labsize(small)) xline(1.5 2.5 3.5 4.5 5.5, lpattern(-) lcolor(gs15)) msize(small) legend(pos(6) row(1)) 


esttab m1 m2 m3 using TractTable3a.tex, tex replace b(%5.3f) se(%5.3f) star(* .1 ** .05 *** .01) label
...
coefplot (m1, label(Vape) msymbol(circle)) (m2, label(Tobacco) msymbol(diamond)) (m3, label(Cigar) msymbol(square)), levels(95) drop(_cons countyfemalepct) vertical yline(0, lwidth(thin)) title("", margin(0 0 2 0)) xlab(,labsize(small)) ylab(,labsize(small)) xline(1.5 2.5 3.5 4.5 5.5, lpattern(-) lcolor(gs15)) msize(small) 
*/
estimates drop _all 
...
use CountyRegVapeSmokeCigarFinal.dta, clear 
/*
***Table 2***
estimates drop _all 
...
**N = 3,141 matching number of counties in US minus Valdez, Kusilvak and Copper River AK (dropped)**
estimates drop _all 
...
sum countyvape countytobacco countycigar [aweight=countyPop]

***Table 2, Panel B***
estimates drop _all 
merge 1:1 countyfips using "Y:\Landini\Figures\CountySmoking.dta"
drop _merge 
merge 1:1 countyfips using "Y:/Landini/GoogleLocations/Matched/GoogleCounty.dta"
drop _merge 
gen unmatchedvape = countyvapegoogle-countyvape 
gen unmatchedtobacco = countytobaccogoogle-countytobacco
gen unmatchedcigar = countycigargoogle-countycigar 
sum unmatched*

estimates drop _all 
areg smoking_rate countyvape countytobacco countycigar county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m2

areg smoking_rate countyvape countytobacco countycigar unmatchedvape unmatchedtobacco unmatchedcigar county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m1

areg smoking_rate lncountyvapevisitors lncountytobaccovisitors lncountycigarvisitors county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m4

areg smoking_rate unmatchedvape unmatchedtobacco unmatchedcigar lncountyvapevisitors lncountytobaccovisitors lncountycigarvisitors county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m3

label var smoking_rate "Smoking Rate"
label var countyvape "\# Vape Matches"
label var countytobacco "\# Smoke Matches"
label var countycigar "\#Cigar Matches"
label var unmatchedvape "\# Unmatched Vape"
label var unmatchedtobacco "\# Unmatched Smoke"
label var unmatchedcigar "\# Unmatched Cigar"
label var countyvapegoogle "\# Google Vape Shops"
label var countytobaccogoogle "\# Google Smoke Shops"
label var countycigargoogle "\# Google Cigar Shops"
label var lncountyvapevisitors "ln(Vape Visitors)"
label var lncountytobaccovisitors "ln(Smoke Visitors)"
label var lncountycigarvisitors "ln(Cigar Visitors)"
label var county1524pct "Age 15-24 \%"
label var countyminoritypct "Minority \%"
label var countyfemalepct "Female \%"
label var countypovertypct "Poverty \%"
label var rural "Rural"
label var countyHSorLess "HS or Less \%"
label var countyvapegoogle "Total"
esttab m1 m2 m3 m4 using Prevalence1.tex, tex replace b(%5.3f) se(%5.3f) star(* .1 ** .05 *** .01) stats(N r2_a) label

areg smoking_rate countyvape countytobacco countycigar [aweight = countyPop], absorb(state) robust
areg smoking_rate lncountyvapevisitors lncountytobaccovisitors lncountycigarvisitors [aweight = countyPop], absorb(state) robust 

estimates drop _all 
areg smoking_rate countyvape countytobacco countycigar [aweight = countyPop], absorb(state) robust 
est sto m2

areg smoking_rate countyvapegoogle countytobaccogoogle countycigargoogle unmatchedvape unmatchedtobacco unmatchedcigar [aweight = countyPop], absorb(state) robust 
est sto m1

areg smoking_rate lncountyvapevisitors lncountytobaccovisitors lncountycigarvisitors countyHSorLess rural [aweight = countyPop], absorb(state) robust 
est sto m4

areg smoking_rate unmatchedvape unmatchedtobacco unmatchedcigar lncountyvapevisitors lncountytobaccovisitors lncountycigarvisitors [aweight = countyPop], absorb(state) robust 
est sto m3

esttab m1 m2 m3 m4 using Prevalence2.tex, tex replace b(%5.3f) se(%5.3f) star(* .1 ** .05 *** .01) stats(N r2_a) label

***Version of Table 2 for only RML/MML states***
gen statefip = state
merge m:1 statefip using "Y:\reusable_files\Marijuana\RML MML dates 2025.dta", keepusing(mml* rml*)
drop _merge 
gen rmlmml = 1 if mml_year != . | rml_year != . 

estimates drop _all 
areg lncountyvapevisitors countyvape countytobacco countycigar county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop] if rmlmml == 1, absorb(state) robust 
est sto m1 
areg lncountytobaccovisitors countyvape countytobacco countycigar county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop] if rmlmml == 1, absorb(state) robust 
est sto m2
areg lncountycigarvisitors countyvape countytobacco countycigar county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop] if rmlmml == 1, absorb(state) robust 
est sto m3

esttab m1 m2 m3 using ImpactCombine2.tex, tex replace b(%5.3f) se(%5.3f) star(* .1 ** .05 *** .01) label stats(N r2_a)

***And for states without***
estimates drop _all 
areg lncountyvapevisitors countyvape countytobacco countycigar county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop] if rmlmml == ., absorb(state) robust 
est sto m1 
areg lncountytobaccovisitors countyvape countytobacco countycigar county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop] if rmlmml == ., absorb(state) robust 
est sto m2
areg lncountycigarvisitors countyvape countytobacco countycigar county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop] if rmlmml == ., absorb(state) robust 
est sto m3

esttab m1 m2 m3 using ImpactCombine3.tex, tex replace b(%5.3f) se(%5.3f) star(* .1 ** .05 *** .01) label stats(N r2_a)

***Version of Table 2 with disaggregated race***
merge 1:1 countyfips using "/cluster/VAST/mp94p-lab/Landini/Figures/CountyDemog.dta"
keep if _merge == 3
drop _merge 

foreach v of varlist Hispanic NonHispanicWA NonHispanicBA NonHispanicAA NonHispanicOther{
gen `v'pct = (`v'/countyPop)*100
}
...
estimates drop _all 
areg lncountyvapevisitors countyvape countytobacco countycigar unmatchedvape unmatchedtobacco unmatchedcigar county1524pct Hispanicpct NonHispanicBApct NonHispanicAApct NonHispanicOtherpct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m1 
areg lncountytobaccovisitors countyvape countytobacco countycigar unmatchedvape unmatchedtobacco unmatchedcigar county1524pct Hispanicpct NonHispanicBApct NonHispanicAApct NonHispanicOtherpct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m2
areg lncountycigarvisitors countyvape countytobacco countycigar unmatchedvape unmatchedtobacco unmatchedcigar county1524pct Hispanicpct NonHispanicBApct NonHispanicAApct NonHispanicOtherpct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m3

esttab m1 m2 m3 using ImpactCombine4.tex, tex replace b(%5.3f) se(%5.3f) star(* .1 ** .05 *** .01) label stats(N r2_a)

***Suggested Appendix Table 
estimates drop _all 
areg lncountyvapevisitors county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m1 
areg lncountytobaccovisitors county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m2
areg lncountycigarvisitors county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m3

coefplot (m1, label(Vape) msymbol(circle)) (m2, label(Smoke) msymbol(diamond)) (m3, label(Cigar) msymbol(square)), levels(95) drop(_cons countyfemalepct) vertical yline(0, lwidth(thin)) title("", margin(0 0 2 0)) xlab(,labsize(small)) ylab(-2(.2).2,labsize(small)) xline(1.5 2.5 3.5 4.5 5.5, lpattern(-) lcolor(gs15)) msize(small) labels

esttab m1 m2 m3 using ImpactCombineA1.tex, tex replace b(%5.3f) se(%5.3f) star(* .1 ** .05 *** .01) label stats(N r2_a)

***Suggested Appendix Table 
estimates drop _all 
areg countyvape county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m1 
areg countytobacco county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m2
areg countycigar county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m3

esttab m1 m2 m3 using ImpactCombineA2.tex, tex replace b(%5.3f) se(%5.3f) star(* .1 ** .05 *** .01) label stats(N r2_a)

**Appendix Table: 2b with visits scaled per 100k 
estimates drop _all 
areg lncountyvapevisitorsper county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m1 
areg lncountytobaccovisitorsper county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m2
areg lncountycigarvisitorsper county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m3

coefplot (m1, label(Vape) msymbol(circle)) (m2, label(Tobacco) msymbol(diamond)) (m3, label(Cigar) msymbol(square)), levels(95) drop(_cons countyfemalepct) vertical yline(0, lwidth(thin)) title("", margin(0 0 2 0)) xlab(,labsize(small)) ylab(,labsize(small)) xline(1.5 2.5 3.5 4.5 5.5, lpattern(-) lcolor(gs15)) msize(small) labels

esttab m1 m2 m3 using ImpactCombineA3.tex, tex replace b(%5.3f) se(%5.3f) star(* .1 ** .05 *** .01) label stats(N r2_a)



***Checking Margins for Table 2*** 
* Model 1: Vape
areg lncountyvapevisitors c.countypovertypct countyvape countytobacco ///
      countycigar county1524pct countyminoritypct countyfemalepct ///
      rural countyHSorLess [aweight = countyPop], absorb(state) robust
margins, at(countypovertypct=(10 15 20 30))
marginsplot, title("Vape Shop Visitors") name(vape, replace)

* Model 2: Tobacco
areg lncountytobaccovisitors c.countypovertypct countyvape countytobacco ///
      countycigar county1524pct countyminoritypct countyfemalepct ///
      rural countyHSorLess [aweight = countyPop], absorb(state) robust
margins, at(countypovertypct=(10 15 20 30))
marginsplot, title("Tobacco Shop Visitors") name(tobacco, replace)

* Model 3: Cigar
areg lncountycigarvisitors c.countypovertypct countyvape countytobacco ///
      countycigar county1524pct countyminoritypct countyfemalepct ///
      rural countyHSorLess [aweight = countyPop], absorb(state) robust
margins, at(countypovertypct=(10 15 20 30))
marginsplot, title("Cigar Shop Visitors") name(cigar, replace)

***Table 2 with Unmatched Counts*** 
foreach v in vape tobacco cigar {
	replace unmatched`v' = 0 if unmatched`v' == 0
	gen total`v' = county`v' + unmatched`v'
}

estimates drop _all
* Model 1: Vape
areg lncountyvapevisitors countyvape countytobacco countycigar unmatchedvape unmatchedtobacco unmatchedcigar [aweight = countyPop], absorb(state) robust
est sto m1

* Model 2: Tobacco
areg lncountytobaccovisitors countyvape countytobacco countycigar unmatchedvape unmatchedtobacco unmatchedcigar [aweight = countyPop], absorb(state) robust
est sto m2

* Model 3: Cigar
areg lncountycigarvisitors countyvape countytobacco countycigar unmatchedvape unmatchedtobacco unmatchedcigar [aweight = countyPop], absorb(state) robust
est sto m3

esttab m1 m2 m3 using ImpactCombine6.tex, tex replace b(%5.3f) se(%5.3f) star(* .1 ** .05 *** .01) label stats(N r2_a)

**With Covariates
estimates drop _all
* Model 1: Vape
areg lncountyvapevisitors countyvape countytobacco countycigar unmatchedvape unmatchedtobacco unmatchedcigar county1524pct countyminoritypct countyfemalepct countypovertypct  rural countyHSorLess [aweight = countyPop], absorb(state) robust
est sto m1

* Model 2: Tobacco
areg lncountytobaccovisitors countyvape countytobacco countycigar unmatchedvape unmatchedtobacco unmatchedcigar county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust
est sto m2

* Model 3: Cigar
areg lncountycigarvisitors countyvape countytobacco countycigar unmatchedvape unmatchedtobacco unmatchedcigar county1524pct countyminoritypct countyfemalepct countypovertypct  rural countyHSorLess [aweight = countyPop], absorb(state) robust
est sto m3

esttab m1 m2 m3 using ImpactCombine5.tex, tex replace b(%5.3f) se(%5.3f) star(* .1 ** .05 *** .01) label stats(N r2_a)

***Suggested Appendix Table 
estimates drop _all 
areg totalvape county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m1 
areg totaltobacco county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m2
areg totalcigar county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m3

esttab m1 m2 m3 using ImpactCombineA3.tex, tex replace b(%5.3f) se(%5.3f) star(* .1 ** .05 *** .01) label stats(N r2_a)

foreach v in totalvape totaltobacco totalcigar {
	gen `v'per = `v'/(countyPop/100000)
	replace `v'per = 0 if `v'per == .
}

***Suggested Appendix Table 
estimates drop _all 
areg countyvapeper county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m1 
areg countytobaccoper county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m2
areg countycigarper county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m3

esttab m1 m2 m3 using ImpactCombineA4.tex, tex replace b(%5.3f) se(%5.3f) star(* .1 ** .05 *** .01) label stats(N r2_a)

estimates drop _all 
areg totalvapeper county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m1 
areg totaltobaccoper county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m2
areg totalcigarper county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m3

esttab m1 m2 m3 using ImpactCombineA5.tex, tex replace b(%5.3f) se(%5.3f) star(* .1 ** .05 *** .01) label stats(N r2_a)

foreach v in unmatchedvape unmatchedtobacco unmatchedcigar {
	gen `v'per = `v'/(countyPop/100000)
	replace `v'per = 0 if `v'per == .
}
replace   
...
***Table 3, Panel B***
estimates drop _all 
areg lncountyvapevisitors countyvapeper countytobaccoper countycigarper unmatchedvapeper unmatchedtobaccoper  unmatchedcigarper county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m1 
areg lncountytobaccovisitors countyvapeper countytobaccoper countycigarper unmatchedvapeper unmatchedtobaccoper  unmatchedcigarper county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m2
areg lncountycigarvisitors countyvapeper countytobaccoper countycigarper unmatchedvapeper unmatchedtobaccoper  unmatchedcigarper county1524pct countyminoritypct countyfemalepct countypovertypct rural countyHSorLess [aweight = countyPop], absorb(state) robust 
est sto m3

esttab m1 m2 m3 using ImpactCombineB.tex, tex replace b(%5.3f) se(%5.3f) star(* .1 ** .05 *** .01) label
/*
...
gen lncountyvisitors = ln(countyvisits)
replace lncountyvisitors = 0 if lncountyvisits == .

maptile lncountyvisitors, geo(county2010) stateoutline(vthin) 
/*
...
merge 1:1 countyfips using "Y:/Landini/GoogleLocations/Matched/CountyStateTotal"
keep if _merge == 3
drop _merge 
drop if missing(countyPop)
***3143 remaining***
...
preserve
***drop smallest 90% of counties***
centile countyPop, centile(90)
foreach v in countyvapematchrate countytobaccomatchrate countycigarmatchrate countyvapevisitors countytobaccovisitors countycigarvisitors {
	replace `v' = . if countyPop < 234054
}
*drop if countyPop < 234054
keep countyfips countyvape countytobacco countycigar countyvapematchrate countytobaccomatchrate countycigarmatchrate countyvapevisitors countytobaccovisitors countycigarvisitors countyPop
export excel using "Y:/Landini/Figures/CountyTobaccoN2.xlsx", replace firstrow(var)
restore 

preserve 
**Drop smallest 90% of states***
gen state = substr(countyfips, 1, 2)
bys state: egen statePop = sum(countyPop)
sum statePop, detail
centile statePop, centile(90)
...
foreach v in statevapematchrate statetobaccomatchrate statecigarmatchrate statevapevisitors statetobaccovisitors statecigarvisitors {
	replace `v' = . if statePop < 13600000
}
*drop if statePop < 13600000
export excel using "Y:/Landini/Figures/StateTobaccoN2.xlsx", replace firstrow(var)
restore 
